function [H, V, E]  = n_spin_spectrum(spins, zfs, ks,  H_mag)
    ...
    for n = 1:length(spins)
        ...
        for l = 1:length(spins)
            s = spin_matrix_z(spins(l))*H_mag;

            if l == 1
                h = s;
            else
                h = kron( kron_id_chain( spin_multiplicities(1:(l-1)) ), s);
            end

            if l ~= length(spins)
                h = kron(h, kron_id_chain( spin_multiplicities(l+1:length(spins)) ) );
            end
